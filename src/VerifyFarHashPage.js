...
function VerifyFarHashPage() {
    // Additional state for user-entered seed and far hash
    const [userSeed, setUserSeed] = useState('');
    const [userFarHash, setUserFarHash] = useState('');
    ...
    const [newFarHashValue, setNewFarHashValue] = useState(''); // State to hold the Far Hash value


    const getSimilarityClass = (score) => {
        if (score >= 75) return 'similarity-high';
        if (score >= 50) return 'similarity-medium';
        return 'similarity-low';
    };

    const verifyFarHash = async (attributes, userFarHash, userSeed) => {

        try {
            const response = await axios.post('http://localhost:5000/verify-far-hash', {
                static: attributes.static,
                dynamic: attributes.dynamic,
                volatile: attributes.volatile,
                far_hash: userFarHash,
                seed: userSeed
            });
            console.log(response.data.similarity);
            setNewFarHashValue(response.data.far_hash); // Update the Far Hash value state


            // Handle response
            if (response.data.similarity > 75) {
                console.log("Verification successful, Far Hash matches.");
            } else {
                console.log("Verification failed, Far Hash does not match.");
            }
            // Update your component's state with the received data
            return response.data; // Contains the far hash and similarity score

        } catch (error) {
            console.error("There was an error verifying the Far Hash:", error);
            // Handle error
            return null;
        }
    ...
    const handleVerifyClick = () => {
        verifyFarHash(attributes, userFarHash, userSeed).then(data => {
            if (data) {
                // Update state based on API response
                setVerificationResult(data.similarity > 75 ? 'Match' : 'No match');
                setSimilarity(data.similarity);

                // You can also update state to show the calculated Far Hash or any other relevant information
            }
        });
    }
    ...
    useEffect(() => {
        const verifyHash = async () => {
            try {
                ...
                if (response.data) {
                    const similarity_score = response.data.similarity;
                    setSimilarity(similarity_score);
                    setNewFarHashValue(response.data.far_hash);

                    if (similarity_score > 75) setVerificationResult('Match');
                    else if (similarity_score > 68 && similarity_score < 75) setVerificationResult('Uncertain');
                    else setVerificationResult('No match');

                    //   setVerificationResult(response.data.similarity > 75 ? 'Match' : 'No match'); // "Match" or "No Match"
                }
            } catch (error) {
        ...
        const timeoutId = setTimeout(() => {
            if (userSeed && userFarHash) {
                verifyHash();
            }
        }, 500); // Adjust debounce time as needed

        return () => clearTimeout(timeoutId);
    }, [attributes.static, attributes.dynamic, attributes.volatile, userSeed, userFarHash]);
    ...
    return (
        ...
        <div>
            ...
            <div className="farHash-page verifyHash-page">
                ...
                <div>
                    <div>
                        ...
                        <input
                            className="attribute-input"
                            value={userFarHash}
                            onChange={e => setUserFarHash(e.target.value)}
                            placeholder="Enter Your Far Hash"
                        />
                    </div>
                ...
                {/* <button onClick={handleVerifyClick}>Verify Far Hash</button> */}
            ...
            <div>
                {similarity !== null && (
                    <div className={`similarity-score ${getSimilarityClass(similarity)}`}>
                        Similarity Score: {similarity}
                        <p class="para"> ({verificationResult})</p>
                    </div>
                )}
...
export default VerifyFarHashPage;
